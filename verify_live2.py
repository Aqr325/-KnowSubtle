"""Self-contained live verification: spawn uvicorn in-process subprocess,
wait for port, run auth checks, terminate. Avoids shell backgrounding issues.
"""
import json
import os
import signal
...
DB = os.path.join(HERE, ".authtest", "live_src.db")
if os.path.exists(DB):
    os.remove(DB)
os.environ["LAS_DB_PATH"] = DB

BASE = "http://127.0.0.1:8753"
proc = subprocess.Popen(
    [sys.executable, "-m", "uvicorn", "app:app", "--host", "127.0.0.1", "--port", "8753"],
    cwd=HERE,
)
time.sleep(1)
...
try:
    ...
    import random
    uname = "live_" + str(random.randint(100000, 999999))
    pw = "Password123"
    results = []
    ...
    # --- chat/config 鉴权强制 (post-fix: 无 token 必须 401) ---
    results.append(check("chat no token->401", req("POST", "/api/chat/agent",
                        body={"message": "hi"})[0], 401))
    ...
    # --- 鉴权通过但 LLM 未配置：应 400（guard 生效，且确实走了鉴权分支）---
    # 注意：tok1/tok2 已被上面的 login2/logout-all 吊销，需重新登录拿有效 token
    s, b = req("POST", "/api/auth/login", body={"username": uname, "password": pw, "remember": False})
