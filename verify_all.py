...
check("chat/agent(empty, 守卫401)", "POST", "/api/chat/agent", body={"agent": "tutor", "message": ""}, expect=401, validator=lambda s, j, r: (isinstance(j, dict) and "detail" in j, "ok"), phase="A")
...
if done or sid:
    ...
    check("chat/agent tutor (守卫400)", "POST", "/api/chat/agent", body={"agent": "tutor", "message": "解释一下词根 bio", "history": []},
          expect=401, validator=lambda s, j, r: (isinstance(j, dict) and "detail" in j, "ok"), phase="D")
