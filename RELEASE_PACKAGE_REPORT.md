# Release-Package 交付报告 · WordCosmos Learning Universe v2.1.0

> **状态：已完成可独立执行的 Windows 桌面程序（全交付 + 全面审计修复）。**
> `Core/main/main.exe`（onedir 单文件夹）已用 PyInstaller 6.21.0 真实编译并**脱离源码独立验证**（全部路由 HTTP 200、离线 demo 流水线端到端跑通并落盘）；`vc_redist.exe` 已内置、`用户手册.pdf` 已生成、并提供免工具一键安装/卸载。

## 一、目录填充对照表（最终状态）

| 目录 / 文件 | 状态 | 说明 |
|-------------|------|------|
| `Core/main/main.exe` | ✅ **已编译** | onedir 单文件夹入口（内部 `main.exe` 约 10.5MB），PyInstaller `--windowed` 桌面版，双击即运行，**无需 Python 环境** |
| `Core/main/_internal/` | ✅ | PyInstaller 6.x 收集的运行依赖（python313.dll + 各 .pyd/.dll），含 `VCRUNTIME140.dll` 与 `local_metagpt`/`learning_agent_system` 子模块 |
...
| `Install/vc_redist.exe` | ✅ **已内置** | 25.6MB，微软 VC++ 2019/2022 x64 可再发行组件，安装脚本静默安装（注：`Core/main/_internal` 已含 `VCRUNTIME140.dll`，Win10/11 通常无需亦可运行） |
| `Docs/LICENSE` | ✅ | MIT 协议 |
| `Docs/用户手册.md` | ✅ | 中文用户手册源文件（路径已更正为 `Core/main/main.exe`） |
| `Docs/用户手册.pdf` | ✅ | 由 `.md` 经 Edge headless 渲染的中文 PDF（579KB，排版正确，PDF 魔法字节校验通过） |
| `Docs/UpdateLog.txt` | ✅ | 更新日志 |
| `Security/FileHash.txt` | ✅ | **529 个文件** SHA256 校验值（含 `Core/main/main.exe` + `_internal` + `vc_redist.exe` + 用户手册.pdf，已按当前产物重算） |
| `Install/install.nsi` | ✅ | NSIS 标准安装脚本（vc_redist 静默安装 + 开始菜单快捷方式指向 `Core/main/main.exe` + 卸载） |
| `Install/install.bat` + `install.ps1` + `uninstall.ps1` | ✅ | **免工具安装器**（无需 NSIS）：右键 `install.bat` 以管理员运行即完成安装/卸载，快捷方式指向 `Core/main/main.exe` |
| `Install/程序图标.ico` | ✅ | 32×32 品牌图标 |

> ⚠️ **打包布局说明**：本次使用 `--onedir --name=main`，PyInstaller 会在 `--distpath Release-Package/Core` 下生成 `Core/main/main.exe`（外层的 `main/` 是 `--name` 目录，内部的 `main.exe` 才是可执行文件）。所有引用路径（安装器、手册、报告）均已统一为 `Core/main/main.exe`。
...
1. **路径双模解析**（`app.py`）：新增 `_resolve_resource_dir/_resolve_config_dir/_resolve_data_dir` + `load_app_config()`。
   - **布局无关**：`_resolve_resource_dir` 从 `_app_root()` 出发**逐级向上查找** `Resources/html`（最多 6 层），兼容 onedir（`Core/main/main.exe` → 向上两级）、onefile、NSIS（`Core` 与 `Resources` 同级）等多种布局，不再硬编码层级。
   - 数据落 `%APPDATA%/WordCosmos/Data`（避免写只读的 Program Files）。
...
5. **原生桌面窗口**（`launcher.py` + `pywebview`）：将启动方式从「打开浏览器」改为「用系统原生 WebView2 渲染桌面窗口」（标题 `WordCosmos 学习宇宙`，1280×800，无地址栏/标签页），呈现真正的桌面应用形态；`pywebview` 已随二进制打包（`_internal/webview`），缺失 WebView2 时自动回退默认浏览器，保证仍可用。依赖 Win10/11 内置的 WebView2 运行时（极少系统需单独安装）。
...
pyinstaller launcher.py --name main --onedir --windowed \
  ...
  --noupx
# 产出直接落 Release-Package/Core/main/（即 Core/main/main.exe），无需手动复制
```
...
脱离项目源码，直接运行 `Release-Package/Core/main/main.exe`（以可写 `APPDATA` 覆盖绕过沙箱限制），结果：

| 请求 | 结果 |
|------|------|
| `GET /api/health` | **HTTP 200** |
| `GET /` | **HTTP 200**，标题 `WordCosmos — Learning Universe` |
...
→ 证明 `main.exe` 能自行定位 `Resources/html`、`Config/`、用户数据目录，启动本地服务并正确响应，是真正可独立执行的桌面端程序；HTML 路由此前因路径解析硬编码层级返回 404，本次已修复并复测通过。

## 五、用户如何使用

### 免安装运行（最简）
双击 `Release-Package/Core/main/main.exe` → 自动启动本地服务并在**原生桌面窗口**（pywebview 渲染，无浏览器）中打开仪表盘。若本机未安装 WebView2 运行时，则自动回退打开默认浏览器访问 `http://127.0.0.1:8000/`。

### 安装到系统（推荐）
- **方式一（免工具，推荐）**：进入 `Release-Package/Install/`，右键 `install.bat` →「以管理员身份运行」。脚本自动复制文件到 `C:\Program Files\WordCosmos Learning Universe`、静默安装 `vc_redist.exe`、创建开始菜单与桌面快捷方式（指向 `Core/main/main.exe`）、写入卸载注册表。
- **方式二（NSIS 标准包）**：在**已装 NSIS** 的机器上执行 `makensis.exe Release-Package\Install\install.nsi`，生成 `WordCosmos-Setup.exe`（输出到 Release-Package 上级，避免被自身递归打包），双击按向导安装。

### 卸载
系统「程序和功能」或运行 `Release-Package/Install/uninstall.ps1`（管理员）。

### 关闭
关闭桌面窗口即退出程序（`main.exe` 同步结束）；若回退到浏览器模式，请关闭浏览器标签并在任务管理器结束 `main.exe`（进程名仍为 `main.exe`；端口冲突已处理 +1 回退）。
...
2. **vc_redist.exe 已内置**（位于 `Install/`）：Win10/11 通常因 `Core/main/_internal` 已含 `VCRUNTIME140.dll` 而无需，纯净/老系统由安装脚本静默安装兜底。
3. **NSIS 安装包未在本机编译**：因本机环境对 sourceforge / GitHub release CDN / Chocolatey 包存储网络不通、winget 的 NSIS 包在用户作用域无可装项，未能自动生成 `WordCosmos-Setup.exe`；已提供等价的免工具 `install.bat`/`install.ps1` 安装器，`install.nsi` 仍保留供有 NSIS 环境编译。
4. **Core/lib/ 为空**：PyInstaller 6.x 将依赖收至 `Core/main/_internal/`（标准结构），与 `lib/` 语义等价。
...
### 后端 / 数据层（已修复）
1. **🔴 打包后 HTML 全部 404（路径解析硬编码）**：`_resolve_resource_dir` 仅查 `<exe>/../Resources/html`，对 onedir（`Core/main/main.exe`）解析到不存在的 `Core/Resources/html`。→ 改为从 `_app_root()` **逐级向上查找** `Resources/html` / `Config`，布局无关，复测 HTML 路由全 200。
...
5. **🟠 安装态只读目录或致启动即 500**：`LongTermMemory` 用相对路径 `./memory/knowledge`，装到 Program Files 时 `os.makedirs` 抛 `PermissionError`。→ 改为接受 `dir_override`，由 `orchestrator` 传入与 `STORAGE_DIR` 一致的 `storage_dir/longterm`；不可写时回退 `AppData/WordCosmos/Data/longterm`。
...
### 收尾（已执行）
- 重打包 `Core/main/main.exe`（22:31:26 版）含全部修复，脱离源码独立验证全绿。
- 修正安装器/文档中 `Core/main.exe` → `Core/main/main.exe` 的所有引用（install.psi、install.nsi 快捷方式、用户手册、Runtime/README）。
...
**结论**：Windows 桌面版已完整交付并经过全面审计修复 —— 可独立运行的 `Core/main/main.exe`（原生 WebView 桌面窗口，非浏览器）+ 内置运行库 + 中文 PDF 手册 + 一键安装/卸载（免工具方案落地，NSIS 方案脚本并存），词库管理已支持学科维度与页面内模态交互。
